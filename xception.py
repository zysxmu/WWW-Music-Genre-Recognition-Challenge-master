...
def xception(inputs,
    ...
    with tf.variable_scope('Xception') as sc:
        ...
        with slim.arg_scope([slim.separable_conv2d], depth_multiplier=1),\
         ...
         slim.arg_scope([slim.batch_norm], is_training=is_training, fused = True):
            ...
            #Block 2
            residual = slim.conv2d(net, 128, [1,1], stride=2, scope='block2_res_conv')
            ...
            #Block 3
            residual = slim.conv2d(net, 256, [1,1], stride=2, scope='block3_res_conv')
            ...
            residual = slim.conv2d(net, 728, [1,1], stride=2, scope='block4_res_conv')
            ...
            #net = tf.nn.relu(net, name='block13_relu1')
            #print(net)
            net = slim.separable_conv2d(net, 1024, [3,3],stride= 2, scope='block13_conv1')
            net = tf.nn.relu(net, name='block13_relu2')
            net = slim.batch_norm(net, scope='block13_bn1')
            net = slim.separable_conv2d(net, 1024, [3,3],stride= 2, scope='block13_conv2')
            net = tf.nn.relu(net, name='block13_relu3')
            net = slim.batch_norm(net, scope='block13_bn2')

            #print(net)
            net = slim.avg_pool2d(net, [5,5], scope='block13_avg_pool')
            #print(net)
            net = slim.dropout(net, keep_prob=keep_prob, is_training=is_training, scope='dropout1')
            net = slim.conv2d(net, 1024, [1,1], scope='block15_conv1')
            logits = slim.conv2d(net, num_classes, [1,1], activation_fn=None, scope='block15_conv2')
...
def xception_arg_scope(weight_decay=0.0001,
...
if __name__ == "__main__":
  print("hello world")
  X = tf.placeholder("float", shape=[1, 299,299,1]) 
  with slim.arg_scope(xception_arg_scope()):
     Y_prediction,_ = xception(X,
                num_classes=16,
                is_training=True,
                scope='xception',
                keep_prob=1.0)
